...
const SankeyBuilder: React.FC = () => {
  ...
  return (
    <div className="flex h-full bg-white text-gray-900 dark:bg-gray-900 dark:text-gray-100 overflow-hidden">
      {/* Sidebar */}
      <aside className="w-80 flex-shrink-0 flex flex-col bg-gray-100 dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 overflow-hidden">
        {/* Header */}
        <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700 bg-gray-850">
          <h1 className="text-sm font-bold text-gray-900 dark:text-white tracking-wide">
          ...
          <button
            ...
            className={`flex-1 py-2 text-xs font-medium transition-colors ${
              sidebarTab === 'input'
                ...
                : 'text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-gray-750'
            }`}
          >
            Input
          </button>
        ...
        {/* Export */}
        <div className="border-t border-gray-200 dark:border-gray-700 p-4 bg-gray-850">
          <ExportButtons
            svgRef={svgRef}
            settings={settings}
            flowsText={inputText}
        ...
        {(errors.length > 0 || imbalances.length > 0) && (
          <div className="border-t border-gray-200 dark:border-gray-700 px-4 py-3 bg-white dark:bg-gray-900 max-h-32 overflow-y-auto">
            {errors.map((e, i) => (
              <p key={i} className="text-xs text-red-600 dark:text-red-400 mb-1">{e}</p>
            ))}
            {imbalances.map((m, i) => (
              <p key={i} className="text-xs text-yellow-700 dark:text-yellow-400 mb-1">Imbalance: {m}</p>
            ))}
          </div>
        )}
      </aside>
